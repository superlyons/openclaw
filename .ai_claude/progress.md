...
```
npm run dev
  └─ scripts/run-node.mjs                       [已读主线 — 1133 行]
       └─ src/entry.ts                          [已读完 — 258 行]
            ├─ tryHandleRootVersionFastPath     [L1 快速路径：--version]
            ├─ tryHandleRootHelpFastPath        [L2 快速路径：根 --help]
            └─ src/cli/run-main.ts (runCli)     [已读到 line 323]
                 ├─ tryRouteCli (line 301-323)  [L3 快速路径：13 个轻量命令]
                 │    └─ src/cli/route.ts       [已读完 85 行]
                 └─ Commander 主流程 (line 327-460)  [⬅ 待读]
