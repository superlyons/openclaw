...
export function loadGlobalRuntimeDotEnvFiles(opts?: { quiet?: boolean; stateEnvPath?: string }) {
  ...
  parsed = [
    {
      filePath: ~/.openclaw/.env 或 cwd()/.env 或 其他明确指定的.env文件路径,
      entries: [
        { key: "OPENCLAW_GATEWAY_PASSWORD", value: "123456" }, ...
      ],
    },
    {
      filePath: ~/.config/openclaw/gateway.env 仅当parsed[0].filePath=="/.openclaw/.env"时才会存在本元素, 代表没有二义性时才会加载gateway.env文件
      entries: [
        { key: "OPENCLAW_GATEWAY_PASSWORD", value: "123456" }, ...
      ],
    },
